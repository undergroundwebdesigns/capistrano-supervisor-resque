      ...
      capistrano_config.load do
        ...
        namespace :resque do
          ...
          task :restart, :roles => lambda { workers_roles() }, :on_no_matching_servers => :continue do
            for_each_workers do |role, workers|
              workers.each do |worker_identifier|
                run("#{(use_sudo) ? "sudo" : ""} kill -s QUIT `#{supervisor_pid_command(worker_identifier)}`", :roles => role)
              end
            end
          end
          ...
          task :resumepause, :roles => lambda { workers_roles() }, :on_no_matching_servers => :continue do
            for_each_workers do |role, workers|
              workers.each do |worker_identifier|
                run("#{(use_sudo) ? "sudo" : ""} kill -s CONT `#{supervisor_pid_command(worker_identifier)}`", :roles => role)
              end
            end
          end
